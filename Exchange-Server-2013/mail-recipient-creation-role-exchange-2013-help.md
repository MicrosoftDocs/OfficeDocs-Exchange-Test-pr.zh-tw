---
title: '郵件建立收件者角色: Exchange 2013 Help'
TOCTitle: 郵件建立收件者角色
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/zh-tw/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50473598
ms.date: 05/21/2018
...
此角色不會讓您擁有郵件功能的公用資料夾。啟用郵件功能的公用資料夾，必須使用`Mail Enabled Public Folders`角色。如需詳細資訊，請參閱[擁有郵件功能的公用資料夾角色](mail-enabled-public-folders-role-exchange-2013-help.md)。

如果您的組織會維護角色存取控制 (RBAC) 分割權限模型的不同群組的使用者可以執行收件者管理收件者建立的執行位置、 指派`Mail Recipient Creation`角色至執行收件者建立、 管理角色群組和`Mail Recipients`角色執行收件者管理角色群組。
...
如需關於 RBAC 和Active Directory分割權限，請參閱[了解分割權限](understanding-split-permissions-exchange-2013-help.md)。
...
除了收件者範圍**New-Mailbox**指令程式，其中包含與此角色，則也受限於使用資料庫組態範圍。資料庫組態範圍控制指令程式可以建立新的信箱的資料庫。您要建立的信箱的資料庫必須是資料庫範圍內。此條件可適用於當您指定使用*Database*參數**New-Mailbox**指令程式上的資料庫或允許自動信箱發佈至您選取的資料庫。如需詳細資訊，請參閱[了解管理角色範圍](understanding-management-role-scopes-exchange-2013-help.md)。
