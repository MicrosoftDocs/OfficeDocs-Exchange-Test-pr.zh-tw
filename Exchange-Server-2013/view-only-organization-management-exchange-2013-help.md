---
title: '僅限檢視組織管理: Exchange 2013 Help'
TOCTitle: 僅限檢視組織管理
ms:assetid: c514c6d0-0157-4c52-9ec6-441d9a30f3df
ms:mtpsurl: https://technet.microsoft.com/zh-tw/library/Dd351130(v=EXCHG.150)
ms:contentKeyID: 50474159
ms.date: 05/21/2018
...
「僅檢視組織管理」管理角色群組是組成 Microsoft Exchange Server 2013 中角色型存取控制 (RBAC) 權限模型的其中一個內建角色群組。角色群組會有一或多個管理角色指派，其中包含執行指定工作集所需的權限。角色群組的成員擁有指派給該角色群組的管理角色存取權。如需角色群組的詳細資訊，請參閱[了解管理角色群組](understanding-management-role-groups-exchange-2013-help.md)。

屬於「僅檢視組織管理」角色群組成員的系統管理員可以檢視 Exchange 組織中任何物件的內容。

這個角色等於 Microsoft Exchange Server 2007 中的 Exchange 僅檢視組織管理角色。
