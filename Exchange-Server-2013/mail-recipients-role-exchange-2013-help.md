---
title: '郵件收件者角色: Exchange 2013 Help'
TOCTitle: 郵件收件者角色
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/zh-tw/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50473579
ms.date: 05/21/2018
...
此角色類型不會讓您管理擁有郵件功能的公用資料夾或通訊群組。若要管理這些物件使用下列角色：
...
如果您的組織有收件者的建立與管理由不同的群組執行其中分割權限模式， `Mail Recipient Creation`將角色指派給執行收件者的建立和執行收件者管理群組之`Mail Recipients`角色群組。如需詳細資訊，請參閱下列主題：
...
除了收件者範圍**Connect-Mailbox**和**Enable-Mailbox**指令程式，亦即包含與此角色，也被範圍使用資料庫組態範圍。資料庫組態範圍控制指令程式可以建立新的信箱的資料庫。您要建立的信箱的資料庫必須是資料庫範圍內。此條件可適用於當您指定*Database*參數使用 cmdlet 或如果您允許自動信箱發佈至您選取的資料庫上的資料庫。如需詳細資訊，請參閱[了解管理角色範圍](understanding-management-role-scopes-exchange-2013-help.md)。
