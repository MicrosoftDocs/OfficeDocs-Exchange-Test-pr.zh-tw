...
<tbody>
<tr class="odd">
<td>使用<strong>Import-UMPrompt</strong>指令程式也匯個別自訂提示。Exchange 2007 UM <strong>Copy-UMCustomPrompt</strong>指令程式不支援將自訂提示複製到 Exchange 2013 UM。</td>
</tr>
</tbody>
</table>
...
UM 撥號對應表會用於整合通訊以確保使用者電話分機是唯一。部分的電話語音網路中多個 IP Pbx、 傳統 Pbx 或已啟用 SIP 的 Pbx 存在。在這些電話語音的網路，可能是兩個使用者具有相同的電話分機號碼。UM 撥號對應表解決所發生此情況。放入兩個不同 UM 撥號對應表的兩個使用者進行其副檔名唯一。如需詳細資訊，請參閱[UM 撥號對應表](um-dial-plans-exchange-2013-help.md)。
...
<tbody>
<tr class="odd">
<td>您可以從 Exchange 2007 UM Server 中使用 <strong>Disable-UMServer</strong> 指令程式，或從 Exchange 2013 Mailbox Server 中使用 <strong>Disable-UMService</strong> 指令程式，來停用自動答錄服務。</td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td>您可以在 Exchange 2007 Unified Messaging server 命令介面或 Exchange 2013 Mailbox server 上的<strong>Set-UMService</strong>指令程式中使用<strong>Set-UMServer</strong>指令程式從單一或多個撥號對應表移除 Exchange 2007 UM 伺服器。例如，以移除所有撥號對應表 UM 伺服器，請執行下列命令： <code>Set-UMServer -identity MyUMServer -DialPlan $null</code></td>
</tr>
</tbody>
</table>
