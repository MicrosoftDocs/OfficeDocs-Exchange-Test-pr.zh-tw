---
title: '法律保留角色: Exchange 2013 Help'
TOCTitle: 法律保留角色
ms:assetid: c98ce8ca-3477-479a-ad23-a8e6459bc4d0
ms:mtpsurl: https://technet.microsoft.com/zh-tw/library/Dd876939(v=EXCHG.150)
ms:contentKeyID: 50474192
ms.date: 05/21/2018
...
`Legal Hold` 管理角色可讓系統管理員設定是否應保留信箱內的資料，以供組織訴訟時使用。
...
除了收件者範圍**Enable-Mailbox**指令程式，其中包含與此角色，則也受限於使用資料庫組態範圍。資料庫組態範圍控制指令程式可以建立新的信箱的資料庫。您要建立的信箱的資料庫必須是資料庫範圍內。這適用於同時指定使用*Database*參數**Enable-Mailbox**指令程式上的資料庫時，或者允許自動信箱發佈至您選取的資料庫。如需詳細資訊，請參閱[了解管理角色範圍](understanding-management-role-scopes-exchange-2013-help.md)。
